 ...
 * @return {number}
 */
//call it jumping back and forth
var maxScore = function (cardPoints, k) {
  let maxScore = 0;

  //sum forst k cards
  let currentSum = cardPoints.slice(0, k).reduce((sum, card) => sum + card, 0);
  maxScore = currentSum;

  //all possibles for sums
  for (let i = 1; i <= k; i++) {
    currentSum += cardPoints[cardPoints.length - i] - cardPoints[k - i];
...
console.log(maxScore([2, 2, 2], 2)); // Expected output: 4
console.log(maxScore([9, 7, 7, 9, 7, 7, 9], 7)); // Expected output: 55
